...
# use git branch named flask_branch to manage this project. Edwin
...
def init_db(dbcon):
    """Initializes the database."""
    with app.open_resource('schema.sql', mode='r') as f:
        dbo.execute_sql(dbcon, f.readlines())
...
def add_entry():
    ...
    with dbo.get_conn(**conn_args) as dbcon:
        with dbcon as cur:
            cur.execute('use flask')
            INSERT_FORMAT= "insert into entries (title, content) values ('{0}','{1}')"
            sql = INSERT_FORMAT.format(request.form['title'], request.form['text'])
            cur.execute(sql)
